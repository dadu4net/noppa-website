...
<div class="modal-backdrop" id="discordModal">
  <div class="modal">
    ...
    <p>Vul een Discord Webhook-URL in en pas het bericht aan. Je vindt de webhook via <strong>Serverinstellingen → Integraties → Webhooks</strong>.</p>
    ...
    <div class="modal-actions">
      ...
      <button class="modal-send" id="discordVerzend" onclick="verzendDiscord()">Versturen →</button>
    </div>
  </div>
</div>
...
<script>
  ...
  function renderArtikel(d) {
    ...
    document.getElementById('pageBody').innerHTML = `
      <section class="hero fade-in">
        <div class="container" style="position:relative;z-index:2">
          <button class="artikel-back" onclick="history.back()">← Terug naar kennisbank</button>
          <div class="hero-eyebrow">
            <svg width="10" height="10" viewBox="0 0 10 10" fill="currentColor" aria-hidden="true"><circle cx="5" cy="5" r="5"/></svg>
            ${d.categorie || 'Artikel'}
          </div>
          ...
          <div class="hero-meta">
            ...
            ${d.auteur   ? `<span style="margin:0 10px">·</span><span>Door ${d.auteur}</span>` : ''}
            ${d.leestijd ? `<span style="margin:0 10px">·</span><span>${d.leestijd}</span>` : ''}
          </div>
        </div>
        ${heroRings()}
      </section>
        ...
        <div class="artikel-layout">
          ...
          <div class="artikel-content-col">
            ...
            <div class="artikel-meta-box">
              ...
              ${d.auteur ? ` &nbsp;·&nbsp; <strong>Auteur:</strong> ${d.auteur}` : ''}
            </div>
          ...
          <div style="margin-top:32px">
            <a href="#/artikelen" class="btn btn-dark">← Alle artikelen</a>
          </div>
  ...
  function kopieerLink(url) {
    navigator.clipboard.writeText(url).then(() => toonToast('✓ Link gekopieerd!'));
  }
  ...
  function kopieerInsta() {
    ...
    navigator.clipboard.writeText(tekst).then(() => {
      toonToast('✓ Tekst gekopieerd! Open Instagram om te plakken.');
  ...
  async function verzendDiscord() {
    ...
    if (!webhook) { toonToast('⚠️ Vul een webhook-URL in'); return; }
    if (!webhook.startsWith('https://discord.com/api/webhooks/')) {
      toonToast('⚠️ Geen geldige Discord webhook-URL'); return;
    }
    if (!bericht) { toonToast('⚠️ Bericht is leeg'); return; }
    ...
    try {
      ...
      if (res.ok) {
        ...
        toonToast('✓ Bericht verstuurd naar Discord!');
        sluitModal('discordModal');
      } else {
        toonToast('✗ Fout: ' + res.status + ' — controleer de webhook-URL');
      }
    } catch(e) {
      toonToast('✗ Versturen mislukt: ' + e.message);
    } finally {
      knop.disabled = false;
      knop.textContent = 'Versturen →';
    }
  ...
  function toonFout(msg) {
    ...
    document.getElementById('pageBody').innerHTML = `
      <div class="container-narrow">
        <div class="fout-box fade-in">✕ ${msg}</div>
      </div>`;
  }
