  ...
  const ok  = async r => { let txt = await r.text(); document.getElementById('debug').innerHTML += '<br>API Response: ' + r.url + ' -> ' + txt.substring(0, 100); if (!r.ok) throw new Error('Niet gevonden HTTP ' + r.status); try { return JSON.parse(txt); } catch(e) { throw new Error('JSON parse fout: ' + e.message); } };
  ...
  function renderArtikelenOverzicht(artikelen) {
    ...
    document.getElementById('pageBody').innerHTML = `
      <div class="artikelen-hero fade-in">
        ...
        <svg class="hero-rings" viewBox="0 0 540 540" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
          <circle cx="270" cy="270" r="269" stroke="rgba(0,208,240,.12)" stroke-width="2"/>
          <circle cx="270" cy="270" r="200" stroke="rgba(0,208,240,.08)" stroke-width="2"/>
        </svg>
      </div>
  ...
  function heroRings() {
    return `<svg class="hero-rings" viewBox="0 0 540 540" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <circle cx="270" cy="270" r="269" stroke="rgba(0,208,240,.15)" stroke-width="2"/>
      <circle cx="270" cy="270" r="210" stroke="rgba(0,208,240,.1)"  stroke-width="2"/>
      <circle cx="270" cy="270" r="150" stroke="rgba(0,208,240,.07)" stroke-width="2"/>
    </svg>`;
  }
  ...
  function toonFout(msg) {
    document.getElementById('debug').innerHTML += '<br>FOUT: ' + msg;
